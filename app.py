...
# -*- coding: utf-8 -*-
"""
Created on Sat Dec 22 16:25:36 2018
...
def callPredict(filename):
   tesla_stocks = pd.read_csv(filename)
   tesla_stocks.head()
   data_to_use = tesla_stocks['values'].values
   ...
   while k+batch_size <= len(X_test):
       o = session.run([model.logits], feed_dict={model.inputs:X_test[k:k+batch_size]})
       k+= batch_size
       tests.append(o)
   ...
   f = Figure(figsize=(10,17),dpi=80)
   a = f.add_subplot(111)
   a.plot(scaled_dataset, label='Original data')
   a.plot(sup, label='Training data')
   #a.plot(test_results, label='Testing data')
   canvas = FigureCanvasTkAgg(f,root)
   canvas.draw()
   canvas.get_tk_widget().pack()
   
tk.Button(root,text="select file",command=selectFile).place(relx=.5, rely=.5, anchor="c")
